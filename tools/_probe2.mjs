...
const out = await page.evaluate((bossId) => {
  ...
  b.chooseTrick = (args) => { calls++; const res = orig(args); if (res) log.push(`POP ${b.ride.combo.names.length} -> ${res.trick}`); return res; };
  for (let i = 0; i < 6000; i++) {
    b.step(C.FIXED_DT, far);
    const ev = b.ride.events.splice(0);
    for (const x of ev) {
      if (x.name === 'trick') log.push(`  land ${x.label} combo=[${b.ride.combo.names.join(',')}]`);
      if (x.name === 'combo') log.push(`  BANK ${x.total}`);
      if (x.name === 'bail') log.push(`  BAIL`);
    }
    if (i % 600 === 599) log.push(`@${((i+1)/120).toFixed(0)}s calls=${calls} mode=${b.ride.mode} tC=${b.trickCool.toFixed(2)} rC=${b.randomTrickCool.toFixed(2)} p=${b.ride.pos.x.toFixed(0)},${b.ride.pos.z.toFixed(0)} v=${Math.abs(b.ride.speed).toFixed(1)}`);
  }
  log.push(`TOTAL calls=${calls}`);
  return log;
}, id);
